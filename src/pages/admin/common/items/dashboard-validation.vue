      ...
      <q-table dense class="no-shadow"
        ...
        <q-tr slot="body" slot-scope="rs" :props="rs"
          ...
          <q-td class="text-right">
            ...
            <br/>
            <span class="text-caption text-faded" v-if="rs.row.created_at">
              {{$app.moment(rs.row.created_at || null).format('D MMM YYYY')}}
            </span>
          </q-td>
        </q-tr>
...
export default {
  ...
  methods: {
    ...
    init() {
      this.loading = true
      let params = ['mode=all', '--with=created_user', 'sampled=SAMPLE:VALIDATE']
