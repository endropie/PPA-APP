    ...
    <q-pull-to-refresh @refresh="TABLE.refresh" inline>
      ...
      >
        ...
        <q-td slot="body-cell-production" slot-scope="rs" :props="rs" style="width:35px">
          <span class="on-left text-small text-grey">({{$app.number_format(100 * rs.row.summary_production / (rs.row.summary_amount || 0))}}%)</span>
          {{rs.value}}
        </q-td>
  ...
  data () {
    return {
      ...
      TABLE: {
        mode: 'index',
        resource: {
          api: '/api/v1/factories/work-orders/lines',
          uri: '/admin/factories/work-orders/lines'
        },
        pagination: {
          rowsPerPage: 0
        },
