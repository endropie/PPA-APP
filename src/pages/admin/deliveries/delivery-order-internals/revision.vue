...
<q-page padding class="form-page justify-center">
  <q-card inline class="main-box" v-if="FORM.show">
    <q-card-section>
      <form-header :title="FORM.title('REVISION')" :subtitle="`# ` + rsForm.fullnumber || rsForm.number || FORM.subtitle()" hide-menu>
        <template  slot="menu-append">
          ...
          <q-chip :label="$tc('form.temporary')" class="text-uppercase" square outline color="orange-10" v-if="rsForm.is_internal" />
        </template>
      </form-header>
    </q-card-section>
    <q-separator inset />
    ...
    <q-card-actions class="q-mx-lg">
      <q-btn outline color="positive" icon="add_circle" :label="$tc('form.add')" @click="addPartition" />
      <q-space/>
      ...
      const submit = () => {
        ...
        this.$axios.set(method, apiUrl, data)
          .then((response) => {
            ...
            setTimeout(() => {
              this.$router.replace(`${this.FORM.resource.uri}/${this.rsForm.id}`)
            }, 500)
          })
          .catch((error) => {
