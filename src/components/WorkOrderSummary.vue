<template>
  <div>
    <div class="column q-gutter-xs q-py-xs" style="min-width:205px" v-if="row && row.summary_productions > 0 && loading === false">
      <div class="row no-wrap items-center text-left">
        ...
        <q-linear-progress size="22px"  color="blue-grey" :value="Boolean(row.summary_items) ? (row.summary_productions/row.summary_items) : 0">
          <div class="absolute-full flex flex-center">
            <q-badge :color="Boolean(row.has_producted) ? 'red' : 'blue-grey-10'" text-color="white" class="text-weight-medium q-pt-xs">
              <span>{{$app.number_abbreviate(row.summary_productions || 0)}} </span>
      ...
      <div class="row no-wrap items-center text-left" v-show="!row.main_id">
        ...
        <q-linear-progress size="22px"  color="blue-grey" :value="Boolean(row.summary_productions) ? (row.summary_packings/row.summary_productions) : 0">
          <div class="absolute-full flex flex-center">
            <q-badge :color="Boolean(row.has_packed) ? 'red' : 'blue-grey-10'" text-color="white" class="text-weight-medium q-pt-xs">
              <span>{{$app.number_abbreviate(row.summary_packings || 0)}} </span>
              <span>&nbsp;/&nbsp;{{$app.number_abbreviate(row.summary_productions || 0)}}</span>
            </q-badge>
          </div>
        </q-linear-progress>
...
export default {
  ...
  mounted () {
    this.loading = true
    this.$axios.get(`/api/v1/factories/work-orders/${String(this.record.id)}`)
      .then((response) => {
